...
static char i2c_write(unsigned char slave_addr, unsigned char reg_addr,
...
#define log_i(...) Linfo(__VA_ARGS__)
 ...
 *  @param[in]  i2c   Raspberry Pi Pico I2C struct
 *  @return     0 if successful.
 */
int mpu_init(i2c_inst_t *i2c) {
    i2c_i = i2c;
...
int mpu_read_6500_accel_bias(long *accel_bias) {
    unsigned char data[6];
    if (i2c_read(st.hw->addr, 0x77, 2, &data[0])) return -1;
    if (i2c_read(st.hw->addr, 0x7A, 2, &data[2])) return -1;
    if (i2c_read(st.hw->addr, 0x7D, 2, &data[4])) return -1;
...
int mpu_read_6050_accel_bias(long *accel_bias) {
    unsigned char data[6];
    if (i2c_read(st.hw->addr, 0x06, 2, &data[0])) return -1;
    if (i2c_read(st.hw->addr, 0x08, 2, &data[2])) return -1;
    if (i2c_read(st.hw->addr, 0x0A, 2, &data[4])) return -1;
    accel_bias[0] = ((long)data[0] << 8) | data[1];
    accel_bias[1] = ((long)data[2] << 8) | data[3];
    accel_bias[2] = ((long)data[4] << 8) | data[5];
    return 0;
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
    unsigned char data[6];
    if (i2c_read(st.hw->addr, 0x13, 2, &data[0])) return -1;
    if (i2c_read(st.hw->addr, 0x15, 2, &data[2])) return -1;
    if (i2c_read(st.hw->addr, 0x17, 2, &data[4])) return -1;
...
int mpu_set_gyro_bias_reg(long *gyro_bias) {
    ...
    if (i2c_write(st.hw->addr, 0x13, 2, &data[0])) return -1;
    if (i2c_write(st.hw->addr, 0x15, 2, &data[2])) return -1;
    if (i2c_write(st.hw->addr, 0x17, 2, &data[4])) return -1;
    return 0;
}
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias) {
    ...
    if (mpu_read_6050_accel_bias(accel_reg_bias)) return -1;
    ...
    if (i2c_write(st.hw->addr, 0x06, 2, &data[0])) return -1;
    if (i2c_write(st.hw->addr, 0x08, 2, &data[2])) return -1;
    if (i2c_write(st.hw->addr, 0x0A, 2, &data[4])) return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
    ...
    if (mpu_read_6500_accel_bias(accel_reg_bias)) return -1;
    ...
    if (i2c_write(st.hw->addr, 0x77, 2, &data[0])) return -1;
    if (i2c_write(st.hw->addr, 0x7A, 2, &data[2])) return -1;
    if (i2c_write(st.hw->addr, 0x7D, 2, &data[4])) return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_read_fifo(short *gyro, short *accel, unsigned char *sensors,
                  unsigned char *more) {
...
static int compass_self_test(void) {
    ...
    data = (short)(tmp[1] << 8) | tmp[0];
    if ((data > 200) || (data < -200)) result |= 0x01;
    data = (short)(tmp[3] << 8) | tmp[2];
    if ((data > 200) || (data < -200)) result |= 0x02;
    data = (short)(tmp[5] << 8) | tmp[4];
    if ((data > -800) || (data < -3200)) result |= 0x04;
#endif
AKM_restore:
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug) {
    ...
    if (debug) log_i("Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
    for (i = 0; i < 3; i++) {
        if (regs[i] != 0) {
    ...
    if (otp_value_zero == 0) {
        if (debug) log_i("ACCEL:CRITERIA A\n");
        for (i = 0; i < 3; i++) {
            ...
            if (fabs(st_shift_ratio[i]) > test.max_accel_var) {
                if (debug) log_i("ACCEL Fail Axis = %d\n", i);
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug) {
    ...
    if (otp_value_zero == 0) {
        if (debug) log_i("GYRO:CRITERIA A\n");
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (fabs(st_shift_ratio[i]) < test.max_gyro_var) {
                if (debug) log_i("Gyro Fail Axis = %d\n", i);
                result |= 1 << i;  // Error condition
            }
        }
    } else {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (st_shift_cust[i] < gyro_st_al_max) {
                if (debug) log_i("GYRO FAIL axis:%d greater than 60dps\n", i);
                result |= 1 << i;  // Error condition
            }
        }
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test,
    ...
    if (debug) log_i("Starting Bias Loop Reads\n");
    ...
    while (s < test.packet_thresh) {
        ...
        if (i2c_read(st.hw->addr, st.reg->fifo_r_w, read_size, data)) return -1;
        ind = 0;
        for (ii = 0; ii < packet_count; ii++) {
            short accel_cur[3], gyro_cur[3];
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug) {
    ...
    if (debug) log_i("Starting MPU6500 HWST!\r\n");
    ...
    if (debug) log_i("Retrieving Biases\r\n");

    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(gyro, accel, 0, debug)) break;
    if (ii == tries) {
        /* If we reach this point, we most likely encountered an I2C error.
         * We'll just report an error for all three sensors.
         */
        if (debug) log_i("Retrieving Biases Error - possible I2C error\n");

        result = 0;
        goto restore;
    }

    if (debug) log_i("Retrieving ST Biases\n");

    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(gyro_st, accel_st, 1, debug)) break;
    if (ii == tries) {
        if (debug) log_i("Retrieving ST Biases Error - possible I2C error\n");
    ...
    if (debug) log_i("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if (debug) log_i("Gyro Self Test Results: %d\n", gyro_result);
    ...
    if (debug) log_i("Compass Self Test Results: %d\n", compass_result);
    if (!compass_result) result |= 0x04;
#else
    result |= 0x04;
#endif
restore:
    if (debug) log_i("Exiting HWST\n");
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
    st.chip_cfg.accel_fsr = 0xFF;
    st.chip_cfg.lpf = 0xFF;
